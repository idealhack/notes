# Python

## Overview

- [Welcome to Python.org](https://www.python.org/)
- [Python - 维基百科](https://zh.wikipedia.org/wiki/Python)

## Documentation

- [3.6.4rc1 Documentation](https://docs.python.org/)
- [The Hitchhiker’s Guide to Python! — The Hitchhiker's Guide to Python](http://docs.python-guide.org/)
- [Python最佳实践指南！ — The Hitchhiker's Guide to Python](http://pythonguidecn.readthedocs.io/)
- [PyFormat: Using % and .format() for great good!](https://pyformat.info/)

## Books

- [Dive Into Python](http://www.diveintopython.net/)

## Videos

- [py-must-watch - Must-watch videos about Python](http://pymust.watch/)
- [Roberto Rosario / django-must-watch · GitLab](https://gitlab.com/rosarior/django-must-watch)

## [[Python Frameworks]]

## [[Python Libraries]]

## Style Guide

- [PEP 8 -- Style Guide for Python Code | Python.org](https://www.python.org/dev/peps/pep-0008/)
- [PEP8: The Style Guide for Python Code](http://pep8.org/)
- [扉页 — Google 开源项目风格指南](http://zh-google-styleguide.readthedocs.io/en/latest/google-python-styleguide/)

## Tools

- [pyenv/pyenv: Simple Python version management](https://github.com/pyenv/pyenv)
- [chriskiehl/Gooey: Turn (almost) any Python command line program into a full GUI application with one line](https://github.com/chriskiehl/Gooey)
- [devpi-latest Documentation](http://doc.devpi.net/latest/)
- [google/grumpy](https://github.com/google/grumpy)

## Resources

- [vinta/awesome-python: A curated list of awesome Python frameworks, libraries, software and resources](https://github.com/vinta/awesome-python)
- [jobbole/awesome-python-cn: Python资源大全中文版，包括：Web框架、网络爬虫、模板引擎、数据库、数据可视化、图片处理等，由伯乐在线持续更新。](https://github.com/jobbole/awesome-python-cn)
- [norvig/pytudes: Python programs to practice or demonstrate skills.](https://github.com/norvig/pytudes)
- [Python Examples](http://www.programcreek.com/python/)
- [Python 3 Module of the Week — PyMOTW 3](https://pymotw.com/3/)
- [kirang89/pycrumbs: Bits and bytes of Python from the Internet](https://github.com/kirang89/pycrumbs)
- [kennethreitz/python-guide: Python best practices guidebook, written for Humans.](https://github.com/kennethreitz/python-guide)
- [humiaozuzu/awesome-flask: A curated list of awesome Flask resources and plugins](https://github.com/humiaozuzu/awesome-flask)
- [Python 黑魔法之 Encoding & Decoding - Thoughts of hsfzxjy - SegmentFault](https://segmentfault.com/a/1190000006037333)
- [Asking for Help/How do you protect Python source code? - Python Wiki](https://wiki.python.org/moin/Asking%20for%20Help/How%20do%20you%20protect%20Python%20source%20code%3F)
- [Welcome to PyInstaller official website](http://www.pyinstaller.org/)
- [使用cProfile等工具来提高python的执行速度-博客-云栖社区-阿里云](https://yq.aliyun.com/articles/91140)

### Socket

- [python的socket编程 | Youmai の Blog](https://michaelyou.github.io/2015/01/31/python%E7%9A%84socket%E7%BC%96%E7%A8%8B/)
- [Python Socket 编程详细介绍 · GitHub](https://gist.github.com/kevinkindom/108ffd675cb9253f8f71)
