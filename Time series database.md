# Time series database

## Overview

- [Time series database - Wikipedia](https://en.wikipedia.org/wiki/Time_series_database)

## Databases

- [influxdata/influxdb: Scalable datastore for metrics, events, and real-time analytics](https://github.com/influxdata/influxdb)
- [prometheus/prometheus: The Prometheus monitoring system and time series database.](https://github.com/prometheus/prometheus)
- [OpenTSDB/opentsdb: A scalable, distributed Time Series Database.](https://github.com/OpenTSDB/opentsdb)

## Resources

- [Introduction to Time Series - Algorithmia Blog](https://blog.algorithmia.com/introduction-to-time-series/)
- [Top 10 Time Series Databases](http://blog.outlyer.com/top10-open-source-time-series-databases)
- [时序列数据库武斗大会之什么是TSDB - 自言自语](http://liubin.org/blog/2016/02/18/tsdb-intro/)
